...
then
    ...
    then
        ...
        log ">> Targetting (Org, Space) = ($CF_ORGANIZATION, $CF_DEV_SPACE)" $SCRIPT_NAME
        cf target -o $CF_ORGANIZATION -s $CF_DEV_SPACE
    elif [ "$1" == "prod" ]
    then
        cf auth $CF_PROD_USERNAME $CF_PROD_PASSWORD
        log ">> Targetting (Org, Space) = ($CF_ORGANIZATION, $CF_PROD_SPACE)" $SCRIPT_NAME
        cf target -o $CF_ORGANIZATION -s $CF_PROD_SPACE
    elif [ "$1" == "staging" ]
    then
        cf auth $CF_STAGING_USERNAME $CF_STAGING_PASSWORD
        log ">> Targetting (Org, Space) = ($CF_ORGANIZATION, $CF_STAGING_SPACE)" $SCRIPT_NAME
        cf target -o $CF_ORGANIZATION -s $CF_STAGING_SPACE
    fi

    log '>> Pushing To CloudFoundry' $SCRIPT_NAME
    cf push
fi
