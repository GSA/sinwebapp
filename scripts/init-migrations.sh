...
then
    ...
    if [ "$#" -ne 1 ]; 
    then
        log "ERROR: No Argument Provided" $SCRIPT_NAME
    else
        log ">> Initializing Django Migrations" $SCRIPT_NAME
        ...
        cd $APP_DIR
        log ">> Project Root: $(pwd)" $SCRIPT_NAME

        # CACHE CUSTOM MIGRATIONS IN /DB/ FOLDER
        log '>> Copying Custom Data Migrations Into \e[4m/db/\e[0m Directory Before Scrubbing The Application' $SCRIPT_NAME
        if [ -f "$APP_DIR/api/migrations/0002_api_data.py" ]
        then
            log '>> Caching 0002_api_data.py' $SCRIPT_NAME
        ...
        then
            log '>> Caching 0003_api_validation.py' $SCRIPT_NAME
        ...
        then
            log '>> Caching 0002_authentication_data.py' $SCRIPT_NAME
            cp -R "$APP_DIR/authentication/migrations/0002_authentication_data.py" "$APP_DIR/db/0002_authentication_data.py"
        fi
        
        # CLEAN MIGRATIONS
        log ">> Cleaning Migrations In All \e[4m$APP_DIR/\e[0m Modules" $SCRIPT_NAME
        if [ -d "$APP_DIR/api/migrations/" ]
        then
            log '>> Cleaning \e[4m/api/migrations/\e[0m Directory' $SCRIPT_NAME
            rm -r $APP_DIR/api/migrations/
        fi
        if [ -d "$APP_DIR/authentication/migrations/" ]
        then
            log '>> Cleaning \e[4m/authenication/migrations/\e[0m Directory' $SCRIPT_NAME
            rm -r $APP_DIR/authentication/migrations/
        fi

        # CREATE FRESH MIGRAITONS
        log ">> Creating New Model Migrations"
        ...
        if [ ! -d "$APP_DIR/api/migrations/" ]
        then
            log '>> No \e[4m/api/migrations/\e[0m Directory Detected, Creating New Directory' $SCRIPT_NAME
            mkdir -p "$APP_DIR/api/migrations/"
            touch "$APP_DIR/api/migrations/__init__.py"
        fi
        if [ ! -d "$APP_DIR/authentication/migrations/" ]
        then
            log '>> No \e[4m/authentication/migrations/\e[0m Directory Detected, Creating New Directory' $SCRIPT_NAME
            mkdir -p "$APP_DIR/authentication/migrations/"
            touch "$APP_DIR/authentication/migrations/__init__.py"
        fi

        log '>> Copying Custom Data Migrations Back Into Application' $SCRIPT_NAME
