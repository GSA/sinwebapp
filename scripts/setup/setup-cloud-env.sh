...
then
    ...
    cd $SCRIPT_DIR/../..
    log ">> OAUTH_SERVICE_ARG: $OAUTH_SERVICE_ARG" $SCRIPT_NAME
    ...
    if [ "$2" == "rebuild" ]
    then
        log '>> Clearing Existing Services' $SCRIPT_NAME
    ...
    log '>> Creating SQL Service' $SCRIPT_NAME
    cf create-service aws-rds medium-psql "$1-sql"

    log '>> Creating S3 Service' $SCRIPT_NAME
    cf create-service s3 basic "$1-s3"

    log '>> Creating OAuth Client Service' $SCRIPT_NAME
    ...
    log '>> Creating \e[3mcloud.gov\e[0m Service Account Credentials'
    cf create-service cloud-gov-service-account space-deployer "$1-account"
    cf create-service-key "$1-account" "$1-key"
    log '>> Please Configure CircleCi Pipeline With The Following Service Credentials.'
    log '>> Store (username, password) => (CF_DEV_USERNAME, CF_DEV_PASSWORD) In CircleCi Environment'
    ...
    do  
        log '>> Waiting On SQL Service Creation' $SCRIPT_NAME
        log '>> SQL Service Status:' $SCRIPT_NAME
        cf service "$1-sql"
        sleep 15s
    done 
    ...
    log '>> Pushing App To Cloud With \e[3m--no-start\e[0m Flag' $SCRIPT_NAME
    cf push --no-start

    log '>> Binding OAuth Client To App' $SCRIPT_NAME
    ...
    log ">> DJANGO SECRET KEY = $SECRET_KEY" $SCRIPT_NAME
    log ">> (CLIENT_ID, CLIENT_SECRET) = ($CLIENT_ID, $CLIENT_SECRET)" $SCRIPT_NAME
    log ">> (DJANGO_SUPERUSER_*) = (USERNAME=$DJANGO_SUPERUSER_USERNAME, EMAIL=$DJANGO_SUPERUSER_EMAIL" $SCRIPT_NAME
    log ">> (EMAIL_HOST, EMAIL_HOST_USER) = ($EMAIL_HOST, $EMAIL_HOST_USER)" $SCRIPT_NAME

    log ">> Setting Environment Variables On Cloud To Printed Values" $SCRIPT_NAME
