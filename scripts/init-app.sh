...
then
    ...
    log ">> \e[3m$1\e[0m Argument Provided, Ensuring Environment Initialized" $SCRIPT_NAME

    if [ "$1" == "local" ]
    then
        log '>> Invoking \e[3minit-env.sh\e[0m Script' $SCRIPT_NAME
        source $SCRIPT_DIR/util/init-env.sh local
    ...
    # by using APP_ENV instead of argument to script. Also need to check for
    # 'local_mcaas' in this case.
    if [ "$APP_ENV" == "container" ] || [ "$APP_ENV" == "local_mcaas" ]
    then 
